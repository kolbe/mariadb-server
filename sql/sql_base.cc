             ...
             TABLE_LIST *table_list, bool *need_prelocking)
{
  ...
  if (table_list->trg_event_map)
  {
    ...
    if (table->file->referenced_by_foreign_key()
        || table->file->has_foreign_keys())
    {
      *need_prelocking= TRUE;

      Query_arena_stmt stmt(thd);
      List <FOREIGN_KEY_INFO> fk_list;
      List <FOREIGN_KEY_INFO> ref_list;

      table->file->get_parent_foreign_key_list(thd, &fk_list);
      if (unlikely(thd->is_error()))
        DBUG_RETURN(TRUE);

      table->file->get_foreign_key_list(thd, &ref_list);
      if (unlikely(thd->is_error()))
        return TRUE;

      prelock_fk_tables(thd, table_list, prelocking_ctx, fk_list, true);
      prelock_fk_tables(thd, table_list, prelocking_ctx, ref_list, false);
    }
  }
